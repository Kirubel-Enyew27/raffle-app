...
test.describe('Wallet Transactions', () => {
  test('should handle deposit and withdrawal successfully', async ({ page }) => {
    ...
    // 3. Verify initial balance is 0.00 Br
    const balanceLocator = page.locator('p.text-3xl')
    await expect(balanceLocator).toHaveText('0.00 Br')

    // 4. Perform Deposit of 100.00 Br
    ...
    // 5. Verify balance updates to 100.00 Br
    await expect(balanceLocator).toHaveText('100.00 Br')

    // 6. Perform Withdrawal of 40.00 Br
    ...
    // 7. Verify balance updates to 60.00 Br
    await expect(balanceLocator).toHaveText('60.00 Br')
    ...
    expect(firstRowText).toContain('40.00 Br') // should show withdrawal of 40.00 Br
  })
})
